...
setup_grub_env() {
  ...
  if ! grep -q "^PATH=.*$NEW_PATH" /etc/environment; then
    if grep -q "^PATH=" /etc/environment; then
      echo "PATH already exists in /etc/environment, but in a different format. Skipping."
    else
  ...
  if ! grep -q "$NEW_PATH" /root/.bashrc; then
    ...
    echo "PATH added to /root/.bashrc"
  else
    echo "PATH already exists in /root/.bashrc"
  fi
  ...
  if [ ! -f "$PROFILE_SCRIPT" ]; then
    ...
    echo "PATH added to $PROFILE_SCRIPT for all users"
  elif ! grep -q "$NEW_PATH" "$PROFILE_SCRIPT"; then
    echo "export PATH=\"$NEW_PATH:\$PATH\"" >> "$PROFILE_SCRIPT"
    echo "PATH added to $PROFILE_SCRIPT"
  else
    echo "PATH already exists in $PROFILE_SCRIPT"
  fi
  ...
  if [ ! -d /boot/grub ]; then
    mkdir -p /boot/grub
    echo "The /boot/grub directory is created"
  else
    echo "The /boot/grub directory already exists"
  fi
...
echo -e "${BIWhite}❖ Nuklir Autoscript Installer ${NC}"
echo
echo -e "${BIWhite}Script Version ${LIME}v.1.0.0${NC}"
...
print_install "Installing Xray Directory and log files"
mkdir -p /etc/xray
...
print_success "Xray directory and log files"
...
function memasang_nginx() {
    clear
    print_install "Installing Nginx & configuration"
    apt install nginx -y
...
function memasang_paket_dasar() {
    clear
    print_install "Installing the Basic Package"
    export DEBIAN_FRONTEND=noninteractive
    apt update -y
    apt upgrade -y
    apt dist-upgrade -y
...
function memasang_domain() {
    ...
    while true; do
        ...
        if [[ $host == "1" ]]; then
            ...
            echo -e "${BIWhite}Your Subdomain $host1 Set up successfully${NC}"
            echo ""
            break
        elif [[ $host == "2" ]]; then
            ...
            echo -e "${RED}Your Choice Is Invalid! Please Select Number 1 or 2${NC}"
        fi
    done
    
    print_success "Your Domain"
}
...
<b>Expired :</b> $EXPIRE_INFO
<b>=========</b>
<b>Notification From Installer Client...</b>
"
  
  local INLINE_KEYBOARD='{"inline_keyboard":[[{"text":"Telegram","url":"https://t.me/dudulrealnofek"}]]}'
...
function memasang_ssl() {
    clear
    print_install "Installing an SSL Certificate on the Domain"
    ...
    print_success "SSL Certificate on the Domain"
}
...
function memasang_folder_xray() {
    ...
    print_install "Additional Folders For SSH & Xray"
}

function memasang_xray() {
    clear
    print_install "Installing Core Xray Version 25.5.16"
    ...
    print_success "Core Xray Version 25.5.16"
    clear
...
clear
print_install "Installing IP & Quota Limitation Service"
wget -q ${REPO}config/limiter.sh && chmod +x limiter.sh && ./limiter.sh
print_success "IP & Quota Limitation Service"
}
...
memasang_pencadangan() {
  ...
  cat <<EOF | sudo tee -a /etc/msmtprc
defaults
tls on
tls_starttls on
...
EOF
  ...
  echo -e "${YELLOW}Check and enable BBR congestion control...${NC}"
  if grep -q "bbr" /proc/sys/net/ipv4/tcp_available_congestion_control; then
      ...
      sysctl -p /etc/sysctl.d/99-network-tune.conf
      echo -e "${GREEN}BBR congestion control successfully activated${NC}"
  else
      echo -e "${RED}BBR is not available on this kernel${NC}"
  fi
  ...
  echo -e "${YELLOW}Configuring QoS for packet prioritization...${NC}"
...
EOF
  ...
  if [ "$total_ram" -le 4096 ]; then
    echo -e "${YELLOW}RAM detected ${total_ram}MB. Enable 2GB swap for system stability${NC}"
    SWAP_SIZE_MB=2048

    if swapon --show | grep -q "/swapfile"; then
      echo -e "${RED}Swapfile is already active, skip swap creation${NC}"
    else
      echo -e "${CYAN}Create a swap file of ${SWAP_SIZE_MB}MB...${NC}"

      if command -v fallocate >/dev/null && fallocate -l "${SWAP_SIZE_MB}M" /swapfile; then
        echo -e "${GREEN}Successfully using fallocate${NC}"
      else
        echo -e "${YELLOW}Fallocate failed, using dd...${NC}"
    ...
    echo -e "${GREEN}RAM ${total_ram}MB detected as large enough. Skipping swap creation${NC}"
  fi
...
function memasang_fail2ban(){
    ...
    if [ -d "/usr/local/ddos" ]; then
        echo -e "Uninstalling The Previous Version First..."
...
function memasang_profile(){
    clear
    print_install "Installing a Profile"
...
function memasang_sshws(){
    clear
    print_install "Installing Python Websocket"
...
echo -e "${BIWhite}Clean up old HAProxy if any...${NC}"
...
echo -e "${BIWhite}Reinstalling HAProxy...${NC}"
sudo apt update && sudo apt install haproxy -y
echo -e "${BIWhite}Merge Xray certificate into /etc/haproxy/hap.pem...${NC}"
mkdir -p /etc/haproxy
cat /etc/xray/xray.crt /etc/xray/xray.key > /etc/haproxy/hap.pem
echo -e "${BIWhite}Create a new HAProxy configuration...${NC}"
...
EOF
echo -e "${BIWhite}Check HAProxy configuration...${NC}"
haproxy -c -f /etc/haproxy/haproxy.cfg
if [ $? -eq 0 ]; then
    echo -e "${BIWhite}Valid configuration. Starting HAProxy...${NC}"
    systemctl restart haproxy
    systemctl enable haproxy
    echo -e "${BIWhite}HAProxy successfully installed and updated!${NC}"
else
    echo -e "${BIWhite}Invalid configuration. Check the file: /etc/haproxy/haproxy.cfg${NC}"
fi
systemctl restart haproxy
